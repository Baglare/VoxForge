...
$CreateProfileScript = Join-Path $ProjectRoot "scripts\create_voice_profile.py"
...
if (Test-Path -LiteralPath $ResolvedInputPath -PathType Leaf) {
    $ResolvedInputPath = (Resolve-Path -LiteralPath $ResolvedInputPath).Path
}
...
if (-not (Test-Path -LiteralPath $CreateProfileScript -PathType Leaf)) {
    Write-Error "Voice profile scripti bulunamadi: $CreateProfileScript"
    exit 1
}
...
Write-Host "Profil adi: $Name"
Write-Host "Input ses dosyasi: $ResolvedInputPath"
Write-Host "Calistirilacak script: $CreateProfileScript"

Set-Location -LiteralPath $ProjectRoot
& $PythonExe $CreateProfileScript --name $Name --input $ResolvedInputPath
exit $LASTEXITCODE
