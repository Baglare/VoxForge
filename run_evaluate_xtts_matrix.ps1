...
$MatrixScript = Join-Path $ProjectRoot "scripts\evaluate_xtts_checkpoint_matrix.py"
...
if (-not (Test-Path -LiteralPath $MatrixScript -PathType Leaf)) {
    Write-Error "XTTS matrix evaluation scripti bulunamadi: $MatrixScript"
    exit 1
}
...
Write-Host "Calistirilacak script: $MatrixScript"
