...
$SmokeCheckScript = Join-Path $ProjectRoot "scripts\smoke_check.py"
...
if (-not (Test-Path -LiteralPath $SmokeCheckScript -PathType Leaf)) {
    Write-Error "Smoke check scripti bulunamadi: $SmokeCheckScript"
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Error ".venv icindeki python.exe calistirilamadi. Sanal ortami yeniden olusturup bagimliliklari kurun."
    exit 1
}

Write-Host "Kullanilacak FFmpeg yolu: $FfmpegExe"
Write-Host "Kullanilacak FFprobe yolu: $FfprobeExe"
Write-Host "Calistirilacak script: $SmokeCheckScript"

Set-Location -LiteralPath $ProjectRoot
& $PythonExe $SmokeCheckScript
exit $LASTEXITCODE
