...
$ParamSweepScript = Join-Path $ProjectRoot "scripts\evaluate_xtts_inference_params.py"
...
if (-not (Test-Path -LiteralPath $ParamSweepScript -PathType Leaf)) {
    Write-Error "XTTS inference param sweep scripti bulunamadi: $ParamSweepScript"
    exit 1
}
...
Write-Host "Calistirilacak script: $ParamSweepScript"
