...
$XtTsScript = Join-Path $ProjectRoot "scripts\first_xtts_test.py"
...
if (-not (Test-Path -LiteralPath $XtTsScript -PathType Leaf)) {
    Write-Error "XTTS test scripti bulunamadi: $XtTsScript"
    exit 1
}
...
Write-Host "Calistirilacak script: $XtTsScript"

Set-Location -LiteralPath $ProjectRoot
& $PythonExe $XtTsScript
exit $LASTEXITCODE
