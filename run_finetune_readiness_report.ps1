...
$ReadinessScript = Join-Path $ProjectRoot "scripts\finetune_readiness_report.py"
...
if (-not (Test-Path -LiteralPath $ReadinessScript -PathType Leaf)) {
    Write-Error "Readiness report scripti bulunamadi: $ReadinessScript"
    exit 1
}
...
Write-Host "Calistirilacak script: $ReadinessScript"

Set-Location -LiteralPath $ProjectRoot
& $PythonExe $ReadinessScript --dataset $Dataset
exit $LASTEXITCODE
